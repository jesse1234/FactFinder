...
from utils.auth import login_admin
...
def show_user_info():
    ...
    with st.sidebar:
        ...
        if st.sidebar.button("🔐 Login", use_container_width=True, key='admin_login_sidebar'):
            st.switch_page("pages/4_🔐_admin_login.py")

        if st.sidebar.button("📝 Sign Up", use_container_width=True, key='admin_signup_sidebar'):
            st.switch_page("pages/6_📝_admin_signup.py")

        for _ in range(20):
            st.write("")

        if st.sidebar.button("Are you a user? Log in here", use_container_width=True, key='user_login_sidebar'):
    ...
    with st.form("login_form"):
        ...
        if submit:
            ...
            with st.spinner("Logging in..."):
                success, error = login_admin(email, password)
