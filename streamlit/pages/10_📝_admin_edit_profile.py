...
def show_user_info():
    """Display admin information in sidebar"""
    with st.sidebar:
...
def update_profile(admin_id, data):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.success(json_response.get('message'))

            if 'email' in data:
                st.session_state.user_email = data['email']
            
            if json_response.get('require_relogin'):
                time.sleep(2)
                st.session_state.authenticated = False
                st.session_state.user_email = None
...
def show_profile_page():
    ...
    st.title("Update Admin Profile")

    user_details = get_user_details()

    if user_details:
        with st.form("admin_update_profile_form"):
